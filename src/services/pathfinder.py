...
class Pathfinder:
    ...
    Parent: (State: State | None) -- oppure --> (((position), broken_walls, converted_traps, score): ((position), broken_walls, converted_traps, score) | None)
    """
    ...
    def _extend_neighbors(self, current_x: int, current_y: int, broken_walls: int,
                          ...
                          convertible_traps: int) -> list[Pathfinder.State]:
        neighbors = []
        ...
        for nx, ny in directions:
            ...
            if score >= 1:  # Se lo stato ha almeno punteggio = 1
                new_score -= 1  # Muoviti, e sottrai 1 al punteggio
                if cell.is_walkable():
    ...
    def _reconstruct_path(self, state: Pathfinder.State, parent: Pathfinder.Parent) -> list[tuple[int, int]]:
        if self.DEBUG:
            print("\n TARGET RAGGIUNTO!")
